...
const char *event_id_to_str (event_log_source_t source, uint8_t event_id)
{
	...
	case EVENT_SRC_MODBUS: { 
		if (event_id == EVENTS_MODBUS_RTU_ERROR_RECEIVING) {
			out = event_log_str_modbus_receiving_err;
		}
		break;
	}
	case EVENT_SRC_UMB: {
								  ...
								  uint16_t output_ln)
{
	...
	if (exposed->severity == EVENT_TIMESYNC) {
		out = snprintf (output,
						output_ln,
						"[%s][Log Counter:%u][Seconds since "
						"powerup:%u][RTC_TIMESYNC][DATE:%d/%d/%d][TIME:%d:%d:%d]",
						severity,
		...
		if (exposed->event_str_name != NULL) {
			out = snprintf (
				output,
				output_ln,
				"[%s][Log Counter:%u][Seconds since powerup:%u][Source: %s][Event: %s][param: "
				"0x%X, param2: 0x%X, wparam: 0x%X, wparam2: 0x%X, lparam: 0x%X, lparam2: 0x%X]",
				severity,
				exposed->event_counter_id,
				exposed->event_master_time,
			...
			out = snprintf (output,
							output_ln,
							"[%s][Log Counter:%u][Seconds since powerup:%u][Source: %s][Unknown "
							"event: 0x%X][param: 0x%X, param2: 0x%X, wparam: 0x%X, wparam2: 0x%X, "
							"lparam: 0x%X, lparam2: 0x%X]",
							severity,
							exposed->event_counter_id,
							exposed->event_master_time,
							exposed->source_str_name,
